...
# 信用卡发放数据集
X = np.insert(np.array([[5, 2], [3, 2], [2, 7], [1, 4], [6, 1], [
              4, 5], [2, 4.5]]), 0, 1, axis=1)  # 给每个xi第一列增加1
...
def drhd(w):
    ew0 = np.mean(-y * X[:, 0] * np.exp(-y * (X @ wk)) /
                  (1 + np.exp(-y * (X @ wk))))
    ew1 = np.mean(-y * X[:, 1] * np.exp(-y * (X @ w)) /
                  (1 + np.exp(-y * (X @ w))))
    ew2 = np.mean(-y * X[:, 2] * np.exp(-y * (X @ w)) /
                  (1 + np.exp(-y * (X @ w))))
